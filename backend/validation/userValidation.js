...
exports.validate=(method)=>{
    switch(method){
        ...
        case 'register_user':{
            return[
                check('name').notEmpty().withMessage('Name is required').isLength(3).withMessage('Enter a valid name').isAlpha().withMessage('Name should not contain numbers or special characters'),
                check('email').notEmpty().withMessage('Email is required'),
                check('email').isEmail().withMessage('Email is not valid'),
